...
> | **② .pbix File** | A single `.pbix` file (I extract it automatically using pbixray) | No need to open PBI Desktop — just hand me the file | Requires the `pbixray` Python package; extraction is best-effort for complex models; **TMDL cleanup is not available** — the extracted TMDL/JSON files are programmatic reconstructions, not the original PBI Desktop source files, so editing them has no effect on your model. You still get DROP SQL and the full optimization report. | Quick analysis, one-off audits, when you can't open PBI Desktop |
